...
namespace PopulationTask
{
    public static class Population
    {
        ...
        public static int GetYears(int initialPopulation, double percent, int visitors, int currentPopulation)
        {
            ...
            if (visitors < 0)
            {
                throw new ArgumentException(" ", nameof(visitors));
            }

            if (currentPopulation <= 0)
            {
                throw new ArgumentException(" ", nameof(currentPopulation));
            }
            ...
            {
                year++;
                initialPopulation = initialPopulation + (int)(((double)initialPopulation * percent) / 100);
